 ...
 function __construct($file_name="")  {
  ...
  function import()  { global $wpdb;
	...
	$wpdb->show_errors     = true;
      ...
      $sql = "LOAD DATA LOCAL INFILE '".esc_sql($this->file_name).
             "' INTO TABLE `".$this->table_name.
             "` CHARACTER SET ".str_replace("-","",$this->encoding)." FIELDS TERMINATED BY '".esc_sql($this->field_separate_char).
             "' OPTIONALLY ENCLOSED BY '".esc_sql($this->field_enclose_char).
             "' ESCAPED BY '".esc_sql($this->field_escape_char).
             "' ".
	    ...
	    if ($wpdb->query($sql) === FALSE) {
			...
			echo "<div class='myerror'><h1>Hosting Needs Updating</h1> <p>This free plugin uses MYSQL \"LOAD DATA INFILE\" to import CSV files. You <strong>MUST</strong> enable this option on your hosting account to continue.</p><p>If you are not sure how todo this, contact your hosting provider and they will do it for you.</p></div>";
			echo "<style>body {color:#fff; } body .myerror { color:#000; }<style>";
			die("");
  ...
  function get_csv_header_fields()
  {
    ...
    if ($fpointer)
    {
      ...
      $this->error = "file cannot be opened: ".(""==$this->file_name ? "[empty]" : esc_sql($this->file_name));
