...
class Permission implements PermissionInterface
{
    ...
    public function validate($fields, $method, $group_id)
    {
        ...
        if ('INSERT' === $method) {
            ...
            throw new MapperException('Not enough permissions to import record. (' . $permission_method . ')');
        }

        if (false === $this->allowed_method($permission_method)) {
		 
            throw new MapperException('Not enough permissions to ' . $permission_method . ' record. (' . $permission_method . ')');
        }
