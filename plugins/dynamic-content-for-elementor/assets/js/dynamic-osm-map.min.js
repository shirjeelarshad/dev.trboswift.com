...
if(this.latLonObject.length==1){this.markers[0].addTo(this.map);(this.circles)&&this.circles[0].addTo(this.map)}else{this.fireFitBound()}}
...
createCircles(){const promises=this.latLonObject.map(async(obj)=>{let data=await this.buildCircle(obj);if("OK"==data.status){return data.circle};if("NOT"==data.status){console.log('Error to create marker')}});return Promise.all(promises)}
async buildCircle(obj){return new Promise((resolve,reject)=>{var circle=L.circle({lon:obj.lon,lat:obj.lat},{radius:this.elementSettings.radius_circles.size,color:this.elementSettings.color_circles,weight:this.elementSettings.weight_circles.size,opacity:this.elementSettings.opacity_circles.size})
if(circle){resolve({status:"OK",circle})}else{resolve({status:"NOT",circle:undefined})}})}
...
let data=await this.buildMarker(obj,custom_icon,custom_infowindow_text);if("OK"==data.status){return data.marker};if("NOT"==data.status){console.log('Error to create marker')}});return Promise.all(promises)}
async buildMarker(obj,custom_icon,custom_infowindow_text){let optionObj={};if(custom_icon){optionObj.icon=custom_icon}
return new Promise((resolve,reject)=>{var marker=L.marker({lon:obj.lon,lat:obj.lat},optionObj)
this.markerClusterGroup.addLayer(marker);marker.on('click',(e)=>{var infoWindow=(custom_infowindow_text)?custom_infowindow_text:`Lat: ${e.latlng.lat} , Lon : ${e.latlng.lng}`;marker.bindPopup(infoWindow);var popup=marker.getPopup();if(popup.isOpen()){marker.openPopup()}})
if(marker){resolve({status:"OK",marker})}else{resolve({status:"NOT",marker:null})}})
return marker}
getLatLonFromAddress(address){return new Promise((resolve,reject)=>{const url=location.protocol+'//nominatim.openstreetmap.org/search?format=jsonv2&q='+address;fetch(url).then((response)=>response.json()).then((data)=>{if(data.length>0){let latlon={lat:data[0].lat,lon:data[0].lon}
resolve({status:"OK",results:latlon})}else{reject({status:"NOT",results:"empty address"})}})})}
...
if("NOT"==data.status){console.error('Error build data from address')
console.log(data.results)}});return Promise.all(promises)}
