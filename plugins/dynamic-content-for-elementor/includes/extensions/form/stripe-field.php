...
class StripeField extends \ElementorPro\Modules\Forms\Fields\Field_Base
{
    ...
    private function is_subscription($id, $record)
    {
     ...
     * @return void
     */
    private function set_subscription_default_payment_method($id)
    {
        if (\strpos($id, 'sub_') !== 0) {
            return;
        }
        ...
        \DynamicOOOS\Stripe\Subscription::update($id, ['default_payment_method' => $pm_id]);
    }
    public function process_field($field, Classes\Form_Record $record, Classes\Ajax_Handler $ajax_handler)
    {
        ...
        if ($this->is_subscription($id, $record)) {
            $this->set_subscription_default_payment_method($intent_id);
            return;
