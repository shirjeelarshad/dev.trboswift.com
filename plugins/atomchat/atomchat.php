...
if( !function_exists( 'atomchatRegisterSettings' ) ) {
	function atomchatRegisterSettings() {
		...
		foreach ($roles as $key => $value) {
			...
			$role->add_cap( 'manage_credit_balance', true );
		}
...
if( !function_exists( 'atomchatDeductPointsCallback' ) ) {
	function atomchatDeductPointsCallback() {
		...
		$server_secret = $data->secretkey;
		$randomString = $data->randomString;
		$client_secret = md5($auth_key.$randomString."dcomdemaetand");
...
if(is_plugin_active('mycred/mycred.php')){
	add_action( 'rest_api_init', function() {
		register_rest_route( 'plugins/mycred', 'credits', [
			...
			'permission_callback' => 'atomchatDeductPointsPermissionCheck',
		]);
	});
}

function atomchatDeductPointsPermissionCheck(WP_REST_Request $request) {
	$data = json_decode(file_get_contents('php://input'));
	$user_id = $data->UID;
	if( $user_id ){
		$user = get_user_by( 'id', $user_id );
		if( $user && user_can($user, 'manage_credit_balance')){
			return true;
		}
	}
	return false;
