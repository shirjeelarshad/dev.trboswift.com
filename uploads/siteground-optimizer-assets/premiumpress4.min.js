jQuery(document).ready(function(){if(jQuery('.rating-label').lenght>0){jQuery('.rating-label').rating();jQuery('.rating-label').each(function(){jQuery('<span class="label label-default"></span>').text(jQuery(this).val()||' ').insertAfter(this)});jQuery('.rating-label').on('change',function(){jQuery(this).next('.label').text(jQuery(this).val())})}})
function ajax_saverating(pid,rr,div){jQuery.ajax({type:"POST",url:ajax_site_url,data:{action:"rating_likes",pid:pid,value:rr,},success:function(e){if(e=="none"){}else if(rr=="up"){jQuery('#'+div+' .count').html(e)}else{jQuery('#'+div+' .count').html(e)}},error:function(e){}})}
