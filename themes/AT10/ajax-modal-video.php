...
if( THEME_KEY == "vt" && !$CORE_VIDEO->has_video_access() && _ppt(array('lst','videopreview_enable')) != "1" ){ // 

?>
 
<div class="card-search card-zoom card-top-image clearfix card-1 ">
    <figure> <a href="javascript:void(0)"  <?php if(!$userdata->ID){ ?> onclick="processLogin();" <?php }else{ ?>onclick="processUpgrade();"<?php } ?>>
...
<div class="bg-light p-5 border text-center" style="height:300px;">
  <div class=" opacity-5 mb-4 mt-5"><i class="fal fa-video fa-4x"></i></div>
...
<?php if(isset($_POST['pid']) && !isset($GLOBALS['top-video']) && !isset($GLOBALS['global_design2'])  && !isset($_POST['action']) ){   ?><hr /><?php } // mute="yes"  ?> 
 
 
 

<?php  echo do_shortcode('[video src="'.$videos[0]['src'].'" width="'.$width.'" height="'.$height.'" autoplay="0" poster="'.$videos[0]['thumbnail'].'" ][/video]'); ?>
...
<?php if(isset($_POST['pid']) && !isset($GLOBALS['global_design2']) && !isset($GLOBALS['global_design3']) && !isset($_POST['action'])  ){  ?><hr /><?php } ?> 

<?php // echo do_shortcode('[VIDEO-YOUTUBE height=600 post_id='.$_POST['pid'].']'); ?>
...
<?php //echo do_shortcode('[VIDEO-VIMEO height=600 post_id='.$_POST['pid'].']'); ?>
...
if(!$CORE_VIDEO->has_video_access() && _ppt(array('lst','videopreview_enable')) == "1"){
