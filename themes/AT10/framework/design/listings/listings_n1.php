...
add_filter( 'ppt_blocks_args',  	array('block_listings_n1',  'data') );
add_action( 'listings_n1',  	array('block_listings_n1', 'output' ) );
add_action( 'listings_n1-css',  array('block_listings_n1', 'css' ) );
add_action( 'listings_n1-js',  	array('block_listings_n1', 'js' ) );

class block_listings_n1 {
	...
	public static function data($a){ global $CORE;  
		...
		$settings = $CORE->LAYOUT("get_block_settings_defaults", array("listings_n1", "listings", $settings ) );
...
?>
            <li> <a href="<?php echo get_term_link( $cat->term_id, $taxonomy); ?>" class="text-dark"> <span class="icon-wrap"><i class="<?php echo $caticon; ?> text-primary"></i></span> <span><?php echo $CORE->GEO("translation_tax", array($cat->term_id, $cat->name)); ?></span> <span class="cat-count cat-count btn btn-system btn-sm">
...
.block-listings_n1 .owl-theme .owl-nav {
    margin-top: 10px;
    position: absolute;
    top: 40%;
	width: 100% !important;
...
.block-listings_n1 .owl-theme .owl-next { float:right; background: #fff !important;    width: 40px; }


.block-listings_n1 .owl-theme  .owl-prev { float:left; background: #fff !important;    width: 40px; }
...
.block-listings_n1 .new-search figure { max-height:400px; }
.block-listings_n1 .new-search figure img { width:100%; }
</style>
<?php
		$output = ob_get_contents();
		ob_end_clean();
