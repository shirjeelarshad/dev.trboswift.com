...
<div class="sidebar-fixed-content">
  <div class="card card-listing">
    <div class="card-body">
      ...
      <a href="javascript:void(0);" <?php if($userdata->ID){ ?>onclick="BuyNowOffer();"<?php }else{ ?>onclick="processLogin();"<?php } ?>  class="btn btn-block btn-system shadow-sm btn-xl btn-icon icon-before mt-2"><i class="fal fa-box-usd  text-primary"></i> <?php echo __("Buy Now","premiumpress"); ?> </a>
      <?php }else{ ?>
      <a href="javascript:void(0);" <?php if($userdata->ID){ ?> onclick="processMakeOffer();"<?php }else{ ?>onclick="processLogin();"<?php } ?> class="btn btn-block btn-system shadow-sm btn-xl btn-icon icon-before mt-2"> <i class="fal fa-comments-alt-dollar text-primary"></i> <span><?php echo $CORE->LAYOUT("captions","offerbtn"); ?></span></a>
      <?php } ?>
      <?php } ?>
