...
<div class="position-relative <?php if(!isset($GLOBALS['nocardwrap'])){ ?>card card-body card-top-image mb-4<?php } ?> top-gallery">
  ...
  <div class="single-images-slider2-nav clearfix">
    ...
    <div class="gallery-item">
      ...
      <?php if(!isset($GLOBALS['nocardwrap'])){ ?>
      <a href="<?php echo $f['src']; ?>" data-toggle="lightbox" data-gallery="example-gallery" data-type="image">
      
        <div class="read_more"><span><?php echo __("view","premiumpress"); ?></span></div>
        </a> 
        <?php } ?>
        
        <img <?php if(!isset($preview)){ ?>data-<?php } ?>src="<?php if($filecount == 1){ echo $f['src']; }else{ echo $f['thumbnail']; } ?>" class="img-fluid lazy" alt="<?php if(isset($f['alt']) && strlen($f['alt']) > 1){ echo stripslashes(strip_tags($f['alt']));  }else{ ?>image <?php echo $i; ?><?php } ?>"> 
