  ...
  <div class="card card-listing">
    <div class="card-body">
      <div class="text-center py-4 mt-2 bg-light mb-4">
        <div class="h1 text-shadow-white price-value <?php echo $CORE->GEO("price_formatting",array()); ?>"><?php echo do_shortcode('[PRICE]'); ?></div>
     ...
     <?php if(in_array(_ppt(array('design','display_offers')), array("","1")) ){ ?>
      ...
      <a href="javascript:void(0);" <?php if(!$userdata->ID){ ?> onclick="processLogin();" <?php }else{ ?>onclick="processMakeOffer();"<?php } ?> class="btn btn-block btn-system shadow-sm btn-xl btn-icon icon-before mt-2 mb-3"> <i class="fal fa-briefcase text-primary"></i> <span><?php echo $CORE->LAYOUT("captions","offerbtn"); ?></span></a>
      <script>
      ...
      <div class="extra-modal-wrap shadow hidepage" style="display:none;">
        ...
        <div class="extra-modal-item">
          <div class="extra-modal-container">
            <div class="card-body">
              <?php _ppt_template( 'framework/design/singlenew/blocks/sidebar-rt-offer' );  ?>
            </div>
