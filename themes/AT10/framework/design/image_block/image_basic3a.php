...
add_filter( 'ppt_blocks_args',  	array('block_image_basic3a',  'data') );
add_action( 'image_basic3a',  		array('block_image_basic3a', 'output' ) );
add_action( 'image_basic3a-css',  	array('block_image_basic3a', 'css' ) );

class block_image_basic3a {
	...
	public static function data($a){ 
		...
		return $a;
		 ...
		 $settings = $CORE->LAYOUT("get_block_settings_defaults", array("image_basic3a", "image_block", $settings ) );
...
<?php $i=1; while($i < 4){ ?>
<div class="<?php if($settings['section_w'] == "container"){ ?> col-sm-4 col-6 <?php }else{ ?>col-md-4 col-6<?php } ?>  <?php if($i == 3){ ?>d-none d-md-block<?php } ?>">
